...
export function KpiCard({ title, value, description, icon, tone = "blue", critical = false }: KpiCardProps) {
  ...
  return (
    <article
      className={cn(
        "group relative min-h-[132px] overflow-hidden rounded-2xl border p-md shadow-[0_18px_42px_rgba(15,23,42,0.09)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_22px_52px_rgba(15,23,42,0.13)] max-lg:min-h-[118px] max-lg:p-sm",
        critical
          ? "border-amber-300/45 bg-[linear-gradient(135deg,#101827_0%,#0D1117_52%,#05070D_100%)] text-white ring-1 ring-white/[0.05]"
          : cn("border-white/90 bg-gradient-to-br ring-1 ring-slate-200/70", styles.card)
      )}
    >
      ...
      <div className="relative flex h-full flex-col justify-between gap-md">
        <div className="flex items-start justify-between gap-md">
          <p className={cn("font-etiqueta text-[12px] font-bold uppercase leading-4 tracking-wide", critical ? "text-amber-100" : "text-slate-600")}>
            {title}
          </p>
          <span
            className={cn(
              "grid h-10 w-10 shrink-0 place-items-center rounded-2xl ring-1 transition-transform group-hover:scale-105",
              critical ? "bg-orange-400 text-slate-950 ring-orange-200 shadow-[0_0_22px_rgba(251,146,60,0.32)]" : styles.icon
            )}
          >
            {icon}
          </span>
        ...
        <div>
          <p className={cn("font-titulo-principal text-[36px] leading-none tracking-normal max-lg:text-[30px]", critical ? "text-orange-300" : styles.value)}>
            {value}
          </p>
          <p className={cn("mt-xs text-sm leading-5 max-lg:text-[12px] max-lg:leading-4", critical ? "text-slate-300" : "text-on-surface-variant")}>
